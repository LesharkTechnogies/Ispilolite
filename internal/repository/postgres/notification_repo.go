...
func (r *notificationRepository) ListNotifications(userID string, unreadOnly bool, limit int) ([]*models.Notification, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ID,&item.UserID,&item.Type,&item.Title,&item.Message,&data,&readAt,&item.CreatedAt); err != nil { return nil, err }
		if readAt.Valid { value := readAt.Time; item.ReadAt = &value }
		if err := json.Unmarshal(data, &item.Data); err != nil { return nil, err }
...
func (r *notificationRepository) MarkNotificationRead(userID, notificationID string) error {
	...
	if err != nil { return err }
	affected, err := result.RowsAffected(); if err != nil { return err }; if affected == 0 { return sql.ErrNoRows }; return nil
}
