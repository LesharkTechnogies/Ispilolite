...
func (s *Service) GetLocation(id string) (*models.Location, error) {
	return s.repo.GetLocationByID(strings.TrimSpace(id))
}

func (s *Service) ListCountyLocations(county string, limit int) ([]*models.Location, error) {
	county = strings.TrimSpace(county)
	if county == "" {
		return nil, fmt.Errorf("county is required")
	}
...
func (s *Service) SubmitLocation(in Submission) (*Result, error) {
	...
	if in.Type == "" {
		in.Type = models.LocationVillage
	}
	...
	if in.Type != models.LocationCounty {
		if in.ParentID == "" {
			return nil, fmt.Errorf("parent_id is required for %s", in.Type)
		}
		parent, err := s.repo.GetLocationByID(in.ParentID)
		if err != nil {
			return nil, err
		}
		if parent == nil {
		...
		if !validParent(in.Type, parent.Type) {
			return nil, fmt.Errorf("%s cannot be a child of %s", in.Type, parent.Type)
		}
		if in.County == "" {
			in.County = parent.County
			if parent.Type == models.LocationCounty {
				in.County = parent.Name
			}
		} else if parent.County != "" && !strings.EqualFold(in.County, parent.County) {
			return nil, fmt.Errorf("location county does not match parent county")
		}
		inside, err := s.repo.ValidateBoundary(parent.ID, in.Latitude, in.Longitude)
		if err != nil {
			return nil, err
		}
		if !inside {
			return nil, fmt.Errorf("coordinates are outside the selected parent boundary")
		}
	}
	...
	if err := s.repo.UpdateLocationStats(location.ID, count, float64(count), verified, status); err != nil {
		return nil, err
	}
...
func validType(kind string) bool {
	return kind == models.LocationCounty || kind == models.LocationTown || kind == models.LocationSubCounty || kind == models.LocationWard || kind == models.LocationVillage
}
func validParent(child, parent string) bool {
	switch child {
	case models.LocationSubCounty, models.LocationTown:
		return parent == models.LocationCounty
	case models.LocationWard:
		return parent == models.LocationSubCounty || parent == models.LocationTown
	case models.LocationVillage:
		return parent == models.LocationWard || parent == models.LocationTown || parent == models.LocationSubCounty
	}
	return false
}
func (s *Service) AddAlias(locationID, userID, alias string) (*models.LocationAlias, error) {
	alias = strings.TrimSpace(alias)
	if locationID == "" || alias == "" {
		return nil, fmt.Errorf("location_id and alias are required")
	}
	location, err := s.repo.GetLocationByID(locationID)
	if err != nil || location == nil {
		return nil, fmt.Errorf("location not found")
	}
	a := &models.LocationAlias{ID: utils.GenerateID(), LocationID: locationID, Alias: alias, CreatedBy: userID, Status: "approved", CreatedAt: time.Now().UTC()}
	if err := s.repo.CreateAlias(a); err != nil {
		return nil, err
	}
	return a, nil
}
