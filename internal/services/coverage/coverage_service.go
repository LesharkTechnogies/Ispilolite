...
func (s *Service) List(ispID, county string) ([]*models.Location, error) { return s.coverage.ListISPCoverage(ispID, strings.TrimSpace(county)) }

func (s *Service) Add(ispID, locationID string) error {
	if strings.TrimSpace(locationID) == "" { return fmt.Errorf("location id is required") }
...
func (s *Service) Recommendations(ispID, county string, limit int) ([]*models.Location, error) {
	...
	for _, place := range places {
		...
		if err := s.notifications.CreateNotification(notification); err != nil { return nil, err }
	}
	return places, nil
}
...
func (s *Service) MarkRead(ispID, notificationID string) error { return s.notifications.MarkNotificationRead(ispID, notificationID) }
